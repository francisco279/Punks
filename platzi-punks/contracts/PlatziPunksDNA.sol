...
//Contrato para implementar el ADN
contract PlatziPunksDNA{
    ...
    //Genera DNA segun el id del token y la direccion del minter (sender de la transaccion)
    //Funcion pura no lee ni escribe datos en blockchain
    function deterministicPseudoRamdomDNA(uint256 _tokenId, address _minter) public pure returns(uint256){
        uint256 combinedParams = _tokenId + uint160(_minter); //Las direcciones siempre usan uint 160
        bytes memory encodeParams = abi.encodePacked(combinedParams);//Convertimos el entero en bytes
        bytes32 hashedParams = keccak256(encodeParams); //Creamos un hash con los encodeParams
    ...
    //funcion internal (Solo para este contrato )
    /* 
    @_dna numero total del DNA
    @_rightDiscard Numeros a la derecha que se descartaran
    La funcion regresara dos numeros de el dna total 
    */
